...
export function MembersInput({
  ...
  return (
    <div>
      <label className="block text-[14px] font-medium mb-2 text-text-on-light">
        Team members{" "}
      ...
      <div className="flex flex-wrap gap-2 mb-2" aria-label="Current members">
        {members.map((m) => (
          <span key={m} className="chip chip-soft">
            {m}
            <button
              type="button"
              aria-label={`Remove ${m}`}
              className="ml-1 text-text-muted-light hover:text-text-on-light"
              onClick={() =>
      ...
      <p
        id="members-help"
        className="mt-1.5 text-[12px] text-text-muted-light"
      >
